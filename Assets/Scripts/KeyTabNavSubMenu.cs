...
public class KeyTabNavSubMenu : MonoBehaviour
{
    ...
    private void SortSelectablesSubMenu()
    {
        List<Selectable> originalSelectables = Selectable.allSelectables;
        List<Selectable> listabala = new List<Selectable>();
        foreach (Selectable button in originalSelectables)
        {
            if (button.transform.parent.tag == "PauseMenu")
            {
                listabala.Add(button);
            }
        }
        int totalSelectables = listabala.Count;
        m_orderedSelectables = new List<Selectable>(totalSelectables);
        for (int index = 0; index < totalSelectables; ++index)
        {
            Selectable selectable = listabala[index];
            m_orderedSelectables.Insert(FindSortedIndexForSelectable(index, selectable), selectable);
        }
    }
